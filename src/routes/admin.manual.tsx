...
const initial: ManualGameInput = {
  name: "", slug: "", emoji: "🎮", description: "", category: "puzzle",
  cover_image_url: "", instructions: "", html_content: "", play_url: "",
  offline_ok: false, min_players: 1, max_players: 1,
};
...
function ManualGamePage() {
  ...
  useState(() => {
    if (!loading && user) check().then((r) => setIsAdmin(r.isAdmin)).catch(() => setIsAdmin(false));
    else if (!loading) setIsAdmin(false);
  });

  const set = <K extends keyof ManualGameInput>(key: K, value: ManualGameInput[K]) =>
    ...
    <main className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <h1 className="font-display text-2xl md:text-3xl font-bold">🎮 手動新增遊戲</h1>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          <section className="border-brutal shadow-brutal rounded-2xl bg-card p-4 md:p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-[1fr_120px] gap-3">
              <Field label="遊戲名稱 *" value={form.name} onChange={(v) => set("name", v)} placeholder="例如：數字消除" />
            ...
            <p className="text-xs text-muted-foreground">遊戲網址會是 /play/網址代稱。只要填英數字和短橫線最穩，畢竟 URL 也不值得拿來寫詩。</p>
            <Field label="簡介" value={form.description} onChange={(v) => set("description", v)} placeholder="簡短介紹遊戲玩法" multiline />
            <div className="grid md:grid-cols-2 gap-3">
              <Field label="分類" value={form.category} onChange={(v) => set("category", v)} placeholder="puzzle / board / logic" />
              <Field label="封面圖片網址" value={form.cover_image_url} onChange={(v) => set("cover_image_url", v)} placeholder="https://..." />
            </div>
            <Field label="遊戲說明" value={form.instructions} onChange={(v) => set("instructions", v)} placeholder="操作方式、勝利條件等" multiline />
          </section>

          <section className="border-brutal shadow-brutal rounded-2xl bg-card p-4 md:p-6 space-y-4">
            <h2 className="font-bold text-lg">② 遊戲內容</h2>
            <div className="rounded-xl bg-muted p-3 text-sm">兩種方式選一種：貼完整 HTML 遊戲，或填可以直接開啟的遊戲網址。</div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <Field label="最少玩家" type="number" value={String(form.min_players)} onChange={(v) => set("min_players", Math.max(1, Number(v) || 1))} />
              <Field label="最多玩家" type="number" value={String(form.max_players)} onChange={(v) => set("max_players", Math.max(1, Number(v) || 1))} />
            </div>
          ...
          <section className="border-brutal shadow-brutal rounded-2xl bg-card p-4 md:p-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <div><b>儲存後會先建立「草稿」</b><p className="text-sm text-muted-foreground">不會直接公開，避免一個手滑把半成品丟給全世界。</p></div>
            <button disabled={busy} className="border-brutal shadow-brutal rounded-xl px-6 py-3 bg-primary text-primary-foreground font-bold disabled:opacity-50">{busy ? "建立中…" : "🎮 建立遊戲草稿"}</button>
          </section>
        </form>
      </div>
    </main>
